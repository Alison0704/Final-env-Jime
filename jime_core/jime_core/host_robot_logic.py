...
class HostRobotLogic(Node):
    def __init__(self):
        ...
        # --- ARUCO Setup ---
        # Note: In newer OpenCV, the detector is initialized via ArucoDetector
    ...
    def main_loop(self):
        ...
        if ids is not None:
            for i in range(len(ids)):
                if ids[i][0] == self.table_id:
                    self.table_detected = True
                    # Calculate center of the marker
                    c = corners[i][0]
                    marker_center_x = np.mean(c[:, 0])
                    self.table_x_offset = (marker_center_x - (frame.shape[1]/2)) / (frame.shape[1]/2)
                    
                    # Draw marker feedback
                    cv2.aruco.drawDetectedMarkers(frame, corners, ids)

        # 3. Stream the processed frame
        ...
        if self.state == RobotState.CLIENT_CHECK:
            ...
            if not self.table_detected:
                # Keep spinning right until we see the table marker
                self.send_cmd("RIGHT")
            else:
                # Table seen! Align with it
                if abs(self.table_x_offset) < 0.15:
                    self.send_cmd("STOP")
                    self.get_logger().info("Table Found and Aligned!")
                    # Optional: Move to TABLE_MOVE state here
                else:
                    self.send_cmd("LEFT" if self.table_x_offset < 0 else "RIGHT")
